...
class RubyShortToLongNovelOutput extends RubyFullParameterOutput { // 短縮形を完全形にする（《》→|《》）
    constructor(options={}) {
        super((match, rb, rt, offset, string, groups)=>{
            ...
            console.log('==================================================================================', string[offset-1], rb, opt.begin);
            //for (let i=1; i<21; i++) { // i<rbLeng, '|','｜'===begin
            for (let i=1; i<opt.rbLen+1; i++) { // i<rbLeng, '|','｜'===begin
                ...
                //if (string[offset-i] in opt.begin) { return match; } // Long型なら変換しない（rb文字数上限までの間に開始字|があれば）
    //            if ('|' === string[offset-i]) { return match; } // Long型なら変換しない
    //            if ('｜' === string[offset-i]) { return match; } // Long型なら変換しない
                if ('》' === string[offset-i]) { break; } // 一つ前のルビ変換処理がありその間に|がなければ変換する
            }
            //if ('|' === string[offset-1]) { return match; } // Long型なら変換しない
            console.log('==================================================================================', string[offset-1], rb);
